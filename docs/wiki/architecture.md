...
Wishlist stars measure repository popularity, not an individual Skill rating. Keep one current, dated entry per source repository in `window.WISHLIST_REPOSITORY_SNAPSHOTS` as source-verification evidence, but do not render that number in the per-Skill score position. Wishlist cards must show `Not rated yet` until a hands-on field test produces an individual rating. State when multiple wishlist Skills share the same source.
